...
class ICarl(IncrementalLearner):
    ...
    def __init__(self, args):
        ...
        self._clf_loss = args.get("classification_loss", {'type': 'bce'})
...
#         self._distil_loss = F.binary_cross_entropy_with_logits
    ...
    def _before_task(self, train_loader, val_loader):
        ...
        if self._groupwise_factors:
            ...
            logger.info(f"Group: classifier, lr: {self._lr}.")

            factor = self._groupwise_factors['convnet']
            ...
            logger.info(f"Group: convnet, lr: {self._lr*factor}.")

            if self._network.post_processor is not None:
                params.append({
                    "params": self._network.post_processor.parameters(),
                    "lr": self._lr
                })
                logger.info(f"Group: post_processor, lr: {self._lr}.")
    ...
    def _compute_loss(self, inputs, outputs, targets, onehot_targets, memory_flags):
        ...
        if self._old_model is None:
            if self._clf_loss['type'] == 'ce':
                loss = F.cross_entropy(post_logits, onehot_targets)       
            elif self._clf_loss['type'] == 'bce':
                loss = F.binary_cross_entropy_with_logits(post_logits, onehot_targets)    
            else:
                raise ValueError(
                    "Unknown classification loss selection {}.".format(self._clf_loss['type'])
                )    
                
        else:
            with torch.no_grad():
                old_targets = self._old_model.post_process(self._old_model(inputs)["logits"]).detach()

            if self._clf_loss['type'] == 'ce':
                loss = F.cross_entropy(post_logits, onehot_targets) + \
                    F.binary_cross_entropy_with_logits(post_logits[:,:-self._task_size], torch.sigmoid(old_targets)) 

            elif self._clf_loss['type'] == 'bce':
                new_targets = onehot_targets.clone()
                new_targets[..., :-self._task_size] = torch.sigmoid(old_targets)
