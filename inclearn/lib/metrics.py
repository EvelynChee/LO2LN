...
class MetricLogger:
    ...
    def __init__(self, nb_tasks, nb_classes, increments, kappa=False):
    ...
    def log_task(self, ypreds, ytrue, task_size, zeroshot=False):
        ...
        if self._kappa:
            self.metrics["kappa"].append(quadratic_kappa(ypreds, ytrue))
    ...
    def last_results(self):
        ...
        if "kappa" in self.metrics:
            results.update({"kappa": self.metrics["kappa"][-1]})
...
def quadratic_kappa(output, targets):
    ## BUG: does not consider the reordering when run on shuffled class order
    output = np.argmax(output, axis = 1) #.cpu().numpy()
        
    return round(cohen_kappa_score(output, targets, weights = 'quadratic'), 3)    
