...
def _train(args, start_date, class_order, run_id):
    ...
    metric_logger = metrics.MetricLogger(
        inc_dataset.n_tasks, inc_dataset.n_classes, inc_dataset.increments, args.get('eval_kappa', False)
    )
    ...
    for task_id in range(inc_dataset.n_tasks):
        ...
        metric_logger.log_task(
            ypreds, ytrue, task_size=args.get("increment"), zeroshot=args.get("all_test_classes")
        )
        ...
        if args.get("eval_kappa"):
            logger.info("Current kappa: {}.".format(metric_logger.last_results["kappa"]))
